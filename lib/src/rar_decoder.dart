...
class RAR5 {
  ...
  RAR5(path) {
    ...
    var signature = randomAccessFile.readSync(8).merge(true);
...
class FILE_SERVICE_BLOCK extends Block {
  int EXTRA_AREA_SIZE, DATA_SIZE, FILE_FLAGS, UNP_SIZE, ATTRIBUTES, MTIME, DATA_CRC, COMPRESSION_INFO, HOST_OS, NAME_SIZE;
...
class ARCHIVE_ENCRYPTION extends Block {
  ...
  ARCHIVE_ENCRYPTION(RandomAccessFile file, Header header) : super(header) {
    ...
    KDF_COUNT = file.readSync(1).merge(true);
    SALT = file.readSync(16).merge(true);
    CHECK_VALUE = file.readSync(12).merge(true);
  }
}
